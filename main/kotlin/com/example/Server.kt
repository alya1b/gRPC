...
class Server {
    ...
    private fun stop() {
        ...
        if (server != null) {
            server.shutdown()
            val deadline = System.currentTimeMillis() + 30_000 // 30 seconds in milliseconds
            while (System.currentTimeMillis() < deadline && !server.isTerminated) {
                // Wait for the server to terminate or until the deadline is reached
    ...
    class GreeterImpl : RemoteDBGrpc.RemoteDBImplBase() {
        ...
        override fun mergeTables(request: MergeTablesRequest, responseObserver: StreamObserver<MergeTablesResponse>) {
            val name1 = request?.name1
            val name2 = request?.name2

            if (name1 != null && name2 != null) {
                val success = dbManager.mergeTables(name1, name2)
                val response = MergeTablesResponse.newBuilder().setSuccess(success).build()
                responseObserver?.onNext(response)
                responseObserver?.onCompleted()
            } else {
                // Handle null or missing data in the request
                // For instance, throw an exception or send an error response
                println("Merging Error")
            }
        }
